...
if %errorlevel% neq 0 (
  ...
  powershell -NoProfile -Command ^
    ...
    "$ps.Arguments = '-NoProfile -ExecutionPolicy Bypass -File \"%~dp0install-agent-windows-server.ps1\" -Nombre \"%NOMBRE%\" -Empresa \"%EMPRESA%\"'; " ^
...
powershell -NoProfile -ExecutionPolicy Bypass -File "%~dp0install-agent-windows-server.ps1" -Nombre "%NOMBRE%" -Empresa "%EMPRESA%"
