...
const fecha = (d) => { try { return new Date(d + 'T00:00:00').toLocaleDateString('es-CL', { day: '2-digit', month: 'long', year: 'numeric' }) } catch { return d } }
...
// Vista previa con zoom + Guardar PDF (igual patrón que los informes).
export function previewCotizacion(c) {
  ...
  btns.append(dl, cl); bar.append(left, btns); ov.appendChild(bar)
  const box = document.createElement('div'); box.style.cssText = 'flex:1;overflow:auto;background:#525659;border-radius:10px;padding:14px;display:flex;justify-content:center'
  const ifr = document.createElement('iframe'); ifr.style.cssText = 'width:210mm;min-height:280mm;border:0;background:#fff;box-shadow:0 2px 16px rgba(0,0,0,.5)'
  box.appendChild(ifr); ov.appendChild(box); document.body.appendChild(ov)
