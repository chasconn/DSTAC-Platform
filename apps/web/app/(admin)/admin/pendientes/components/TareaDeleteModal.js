...
export default function TareaDeleteModal({ tarea, onClose, onDeleted }) {
  ...
  async function handleDelete() {
    ...
    try {
      await api.delete(`/api/pending/${tarea.id}`)
      onDeleted()
    ...
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.45)', zIndex: 100, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }}
      ...
      <div style={{ background: '#fff', borderRadius: 14, width: '100%', maxWidth: 360, boxShadow: '0 20px 60px rgba(0,0,0,0.2)', overflow: 'hidden' }}>
        <div style={{ padding: '24px 24px 20px' }}>
          <h2 style={{ margin: '0 0 10px', fontSize: 17, fontWeight: 700, color: '#2C2C2A' }}>Eliminar tarea</h2>
          <p style={{ margin: 0, fontSize: 13, color: '#888780', lineHeight: 1.5 }}>
            ¿Eliminar <strong style={{ color: '#2C2C2A' }}>"{tarea.title}"</strong>? Esta acción no se puede deshacer.
          </p>
