...
export default function Ley21719Page() {
  ...
  async function emitirDesdeHistorial(id) {
    try {
      await api.post(`/api/admin/ley21719/${id}/certificado`, {}, headers)
      showToast('Certificado de cumplimiento emitido')
      cargarHistorial()
    } catch (e) { showToast(e.message || 'No se pudo emitir el certificado') }
  }
                      ...
                      <td style={{ padding: '8px 10px', fontSize: 12, textAlign: 'center', borderBottom: '1px solid #f5f4ef' }}>
                        {h.certificado_codigo ? (
                          ...
                          <div style={{ display: 'inline-flex', gap: 6 }}>
                            <BotonInforme tipo="certificado" slug={slug} label="Vista previa" query={{ evaluacionId: h.id, ley: '21719', preview: 1 }} />
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 12, marginBottom: 18 }}>
            ...
            <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
              {resultado.nivel === 'Alto' && !certResult?.codigo && (
                <>
                  <BotonInforme tipo="certificado" slug={slug} label="👁 Vista previa" query={{ evaluacionId: resultado.id, ley: '21719', preview: 1 }} />
