...
export default function ClienteIncidenteDetalle({ incidente, onClose }) {
  if (!incidente) return null

  const estaAbierto = incidente.estado === 'abierto' || incidente.estado === 'en_investigacion' || incidente.estado === 'en_respuesta'
  const esCritico   = incidente.severidad === 'critica'
    ...
    <aside style={{ width: 280, minWidth: 280, background: '#fff', borderLeft: '1px solid #e2e0d8', display: 'flex', flexDirection: 'column', height: '100%', flexShrink: 0 }}>
      ...
      {esCritico && estaAbierto && (
        <div style={{ background: '#FCEBEB', borderBottom: '1px solid #E8A6A6', padding: '8px 14px', fontSize: 12, color: '#791F1F', fontWeight: 500 }}>
          ⛔ Severidad crítica — Contacta a tu analista DSTAC.
        </div>
      )}
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '0 16px' }}>
        <Row label="Impacto"      value={incidente.impacto ? (IMPACTO_LABEL[incidente.impacto] ?? incidente.impacto) : null} />
        <Row label="Responsable"  value={incidente.responsable} />
        <Row label="Proyecto"     value={incidente.proyecto} />
        {incidente.cvss != null && (
          <Row label="CVSS" value={`${incidente.cvss} / 10`} />
        )}
        <Row
        ...
        {incidente.fecha_respuesta && (
          <Row
            ...
            value={new Date(incidente.fecha_respuesta).toLocaleString('es-CL')}
          />
        )}
        {incidente.tiempo_resolucion != null && (
          <Row label="Tiempo de resolución" value={`${incidente.tiempo_resolucion} min`} />
        )}
        ...
        {incidente.descripcion && (
          <div style={{ padding: '10px 0', borderBottom: '1px solid #f1efe8' }}>
            ...
            <p style={{ fontSize: 13, color: '#2C2C2A', marginTop: 4, lineHeight: 1.5, whiteSpace: 'pre-wrap' }}>{incidente.descripcion}</p>
          </div>
        )}
        ...
        {incidente.causa_raiz && (
          <div style={{ padding: '10px 0', borderBottom: '1px solid #f1efe8' }}>
            ...
            <p style={{ fontSize: 13, color: '#2C2C2A', marginTop: 4, lineHeight: 1.5, whiteSpace: 'pre-wrap' }}>{incidente.causa_raiz}</p>
          </div>
        )}
